...
class ArticleCollectionInfoAdapter : BaseQuickAdapter<ArticleCollectionInfoBean, BaseViewHolder>(R.layout.item_article_collection_info) {

    private val mUnCloseList: ArrayList<SwipeLayout> = ArrayList()

    override fun convert(helper: BaseViewHolder?, item: ArticleCollectionInfoBean) {
        ...
        if (item.blog != null){
            helper?.setText(R.id.tv_article_authorName,"  "+item.favoriteTime)

...
class SwipeCaptchaView @JvmOverloads constructor(context: Context,attrs: AttributeSet?=null, defStyleAttr: Int = 0) : AppCompatImageView(context, attrs, defStyleAttr){

    private val TAG = SwipeCaptchaView::class.java.name
    ...
    private fun createMatchAnim() {
        ...
        mFailAnim?.addListener(object : AnimatorListenerAdapter() {
            override fun onAnimationEnd(animation: Animator) {
                onCaptchaMatchCallback!!.matchFailed(this@SwipeCaptchaView)
            }
        })
        mFailAnim?.addUpdateListener(ValueAnimator.AnimatorUpdateListener { animation ->
            val animatedValue = animation.animatedValue as Float
            TLog.d(TAG, "onAnimationUpdate: $animatedValue")
        ...
        mSuccessAnim?.addListener(object : AnimatorListenerAdapter() {
            ...
            override fun onAnimationEnd(animation: Animator) {
                onCaptchaMatchCallback!!.matchSuccess(this@SwipeCaptchaView)
    ...
    private fun createCaptchaPath() {
        ...
        //draw一个随机凹凸的圆
        drawPartCircle(
            PointF((mCaptchaX + mCaptchaWidth - gap).toFloat(), (mCaptchaY + mCaptchaHeight).toFloat()),
    ...
    private fun getMaskBitmap(mBitmap: Bitmap, mask: Path?): Bitmap {
        ...
        TLog.e(TAG, " getMaskBitmap: width:" + mBitmap.width + ",  height:" + mBitmap.height)
        TLog.e(TAG, " View: width:$mWidth,  height:$mHeight")
        //把创建的bitmap作为画板
    ...
    interface OnCaptchaMatchCallback {
        fun matchSuccess(rxSwipeCaptcha: SwipeCaptchaView)
        fun matchFailed(rxSwipeCaptcha: SwipeCaptchaView)
    }
     ...
     * @return
     */
    fun setOnCaptchaMatchCallback(onCaptchaMatchCallback: OnCaptchaMatchCallback?): SwipeCaptchaView {
